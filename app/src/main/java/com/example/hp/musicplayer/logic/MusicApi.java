...
import com.example.hp.musicplayer.API.ApiService;
import com.example.hp.musicplayer.API.Play;
...
public class MusicApi {
    MusicApi musicApi;
    public static final String baseUrl = "http://tingapi.ting.baidu.com/v1/restserver/";
    ApiService service;
    String url2;
    RequestQueue mQueue;
    Context context;
    ...
    public void setUrl(String name, Context context, ThirdFragment fragment){
    ...
    Search msearchSongList;
    public void playOnlineMusic(int position) {
        String songId = msearchSongList.getSong().get(position).getSongid();
        Observable<Play> observable = service.play("json", null, "webapp_music", "baidu.ting.song.play", songId);
        observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
